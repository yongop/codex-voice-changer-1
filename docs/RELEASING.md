...
GitHub 저장소와 소스 아카이브에는 모델 파일을 넣지 않습니다. 설정 스크립트는
공식 배포처에서 모델을 받되 동의를 기록하지 않으며, 앱 최초 실행 화면에서
RVC 사용 전에 공식 배포 페이지와 현재 규약을 확인하고 명시적으로 동의하도록
합니다.

`Scripts/build-app.sh`로 만든 서명된 앱 ZIP에는 공식 모델이 포함됩니다. 공식
